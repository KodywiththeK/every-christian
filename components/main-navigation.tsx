...
export default function MainNavigation() {
  ...
  return (
    <div className="bg-white border-t border-gray-200 py-2 sticky bottom-0">
      <div className="container max-w-md mx-auto">
        <nav className="flex justify-between items-center">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex flex-col items-center px-4 py-2 ${isActive ? 'text-primary' : 'text-gray-500 hover:text-gray-700'}`}>
                {item.icon}
